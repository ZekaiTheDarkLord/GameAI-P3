...
void Graphics::drawMob(Entity* m)
{
    ...
    if (m->getStats().getName() == "Rogue" && m->isInvisible()) return;
    ...
    if (m->isNorth())
    {
        ...
        {
            SDL_SetRenderDrawColor(gRenderer, 0xFF, 0xA0, 0xA0, alpha);
        }
    }
    else
    {
        ...
        {
            SDL_SetRenderDrawColor(gRenderer, 0xA0, 0xA0, 0xFF, alpha);
        }
    }
